...
app.config['MYSQL_PASSWORD']    = 'nexusadministrator'
...
EMAIL_PASSWORD = 'obuqbmusxnzkrgcr'          # ← 16-char App Password
...
def send_otp_email(to_email: str, otp: str) -> bool:
    ...
    if 'your_gmail' in EMAIL_USER or len(EMAIL_PASSWORD) < 10:
    ...
    try:
        ...
        html_body = f"""
        <!DOCTYPE html><html><body style="margin:0;padding:0;background:#f0f0f8;font-family:Arial,sans-serif;">
          <div style="max-width:480px;margin:40px auto;background:#ffffff;border:1px solid #e0e0f0;border-radius:18px;overflow:hidden;">
            <div style="padding:28px 32px 20px;background:#4f46e5;">
              <div style="font-size:13px;font-weight:700;letter-spacing:0.16em;text-transform:uppercase;color:#c7c2ff;">NEXUS</div>
              <h1 style="font-size:22px;font-weight:700;color:#ffffff;margin:8px 0 0;">Reset your password</h1>
            </div>
            <div style="padding:28px 32px;">
              ...
              <div style="text-align:center;background:#f5f4ff;border:2px solid #c7c2ff;border-radius:12px;padding:24px;margin-bottom:24px;">
                <div style="font-size:38px;font-weight:700;letter-spacing:0.22em;color:#4f46e5;font-family:monospace;">{otp}</div>
              </div>
              <p style="color:#aaaacc;font-size:12px;">Never share this OTP with anyone.</p>
            </div>
        ...
        print("[EMAIL ERROR] Auth failed — use Gmail App Password, not real password.")
        return False
    except Exception as e:
...
def dashboard():
    ...
    # Departments for all roles (needed for report form)
...
def approve(booking_id):
    ...
    mysql.connection.commit()
    cur.close()
    return jsonify({'ok': True, 'msg': 'Booking approved.', 'booking_id': booking_id})
...
def reject(booking_id):
    ...
    mysql.connection.commit()
    cur.close()
    return jsonify({'ok': True, 'msg': 'Booking rejected.', 'booking_id': booking_id})
...
def cancel_booking(booking_id):
    ...
    mysql.connection.commit()
    cur.close()
    return jsonify({'ok': True, 'msg': 'Booking cancelled.', 'booking_id': booking_id})
...
def report_issue():
    ...
    if session['role'] == 'hod':
        ...
        mysql.connection.commit()
        cur.close()
        flash('Issue reported to Admin (Centralised).' if int(dept_id) == 99
              else 'Issue reported to the department HoD.', 'success')
...
def resolve_issue(issue_id):
    if session.get('role') != 'admin':
        return jsonify({'ok': False, 'msg': 'Only Admin can resolve issues.'})

    cur = mysql.connection.cursor()
    cur.execute("SELECT issue_id FROM issues WHERE issue_id = %s", [issue_id])
    if not cur.fetchone():
        cur.close()
        return jsonify({'ok': False, 'msg': 'Issue not found.'})

    cur.execute("UPDATE issues SET status='resolved' WHERE issue_id=%s", [issue_id])
    mysql.connection.commit()
    cur.close()
    return jsonify({'ok': True, 'msg': 'Issue marked as resolved.', 'issue_id': issue_id})
...
def forward_issue(issue_id):
    if session.get('role') != 'hod':
        return jsonify({'ok': False, 'msg': 'Only HoD can forward issues.'})

    cur = mysql.connection.cursor()
    cur.execute("SELECT department_id, status FROM issues WHERE issue_id = %s", [issue_id])
    issue = cur.fetchone()
    ...
    if int(issue['department_id']) != int(session['department_id']):
        cur.close()
        return jsonify({'ok': False, 'msg': 'This issue does not belong to your department.'})
    if issue['status'] != 'open':
        cur.close()
        return jsonify({'ok': False, 'msg': 'Already forwarded or resolved.'})

    cur.execute("UPDATE issues SET status='forwarded', department_id=99 WHERE issue_id=%s", [issue_id])
    mysql.connection.commit()
    cur.close()
    return jsonify({'ok': True, 'msg': 'Issue forwarded to Admin.', 'issue_id': issue_id})
...
def add_resource():
    ...
    try:
        ...
        flash('Database error: could not add resource.', 'error')
        print('[DB ERROR]', e)
    finally:
        cur.close()
